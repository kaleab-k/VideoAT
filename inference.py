...
def inference(data_loader, model, result_path, class_names, no_average,
              output_topk, attack_type="clean", eps=4, step_size=1, attack_iter=5):
    print('inference')
            ...
            if attack_type == "clean":
                ...
                with torch.enable_grad():
                    ...
                    adversary = LinfPGDAttack(predict=model, loss_fn=criterion,
                                             eps=float(eps/255), nb_iter=attack_iter, eps_iter=step_size/255)
                    adv_inputs = adversary.perturb(inputs)
